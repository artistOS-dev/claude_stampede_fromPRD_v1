...
export default async function HomePage() {
  ...
  return (
    <div className="space-y-8">
      {/* Welcome hero */}
      <div className="bg-gradient-to-br from-amber-900 via-stone-800 to-stone-900 rounded-2xl p-8 text-white border border-amber-800/40">
        <div className="flex items-center gap-4">
          <div className="flex-shrink-0">
            {avatarUrl ? (
              <Image
                ...
                width={64}
                height={64}
                className="w-16 h-16 rounded-full object-cover border-2 border-amber-700/50"
              />
            ) : (
              <div
                className="w-16 h-16 rounded-full bg-amber-800/40 flex items-center justify-center text-xl font-bold border-2 border-amber-700/50 text-amber-100"
          ...
          <div>
            <h1 className="text-2xl font-bold font-display text-amber-100">Welcome to Stampede, {displayName}!</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <a href="/circles" className="bg-stone-900 rounded-xl border border-stone-700 shadow-sm p-6 flex items-center gap-4 hover:border-amber-800 transition-colors">
          ...
          <div>
            <p className="text-sm text-stone-500">Your circles</p>
            <p className="text-xl font-bold text-white">{myCircles.length}</p>
          </div>
        </a>

        <div className="bg-stone-900 rounded-xl border border-stone-700 shadow-sm p-6 flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-amber-900/30 flex items-center justify-center flex-shrink-0">
            <Headphones className="w-5 h-5 text-amber-400" aria-hidden="true" />
          </div>
          <div>
            <p className="text-sm text-stone-500">Songs shared</p>
        ...
        <div className="bg-stone-900 rounded-xl border border-stone-700 shadow-sm p-6 flex items-center gap-4">
          ...
          <div>
            <p className="text-sm text-stone-500">Artists following</p>
      ...
      {/* My Circles */}
      {myCircles.length > 0 && (
        <div className="bg-stone-900 rounded-xl border border-stone-700 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-white">My Circles</h2>
            <a href="/circles" className="text-sm text-amber-400 hover:text-amber-300 font-medium">
              Browse more
            </a>
          </div>
          <div className="space-y-2">
            {myCircles.map((circle) => (
              <a
                ...
                className="flex items-center gap-3 p-3 rounded-lg bg-stone-950 border border-stone-800 hover:border-amber-800 hover:bg-amber-950/20 transition-colors group"
              >
                <div className="w-9 h-9 rounded-full bg-amber-900/30 flex items-center justify-center flex-shrink-0">
                  <Music2 className="w-4 h-4 text-amber-400" aria-hidden="true" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-white group-hover:text-amber-400 transition-colors truncate">
                    {circle.name}
                  </p>
                  <p className="text-xs text-stone-500 truncate">{circle.description}</p>
                </div>
              ...
              <div key={song.id} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-stone-500 truncate">
                    {song.artist}
                    {song.circles ? (
                      <> · <a href={`/circles/${song.circles.id}`} className="text-amber-400 hover:text-amber-400">{song.circles.name}</a></>
                    ) : null}
                  </p>
                </div>
                {song.avg_rating > 0 && (
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    <span className="text-xs text-stone-500">{song.avg_rating.toFixed(1)}</span>
                    <span className="text-xs text-stone-600">({song.rating_count})</span>
                  </div>
                )}
              </div>
            ))}
      ...
      <div className="bg-stone-900 rounded-xl border border-stone-700 shadow-sm p-6">
        ...
        <div className="space-y-3">
          <a href="/circles" className="flex items-center gap-3 p-3 rounded-lg hover:bg-stone-800 transition-colors group">
            <div className="w-9 h-9 rounded-full bg-amber-900/30 flex items-center justify-center flex-shrink-0">
              <Users className="w-4 h-4 text-amber-400" aria-hidden="true" />
            </div>
            <div className="flex-1">
              <p className="text-sm font-medium text-white group-hover:text-amber-400 transition-colors">Browse Circles</p>
              <p className="text-xs text-stone-500">Find communities of fans who share your taste</p>
            </div>
