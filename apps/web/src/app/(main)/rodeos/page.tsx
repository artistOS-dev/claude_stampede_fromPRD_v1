...
const TYPE_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  ...
  whale: { bg: 'bg-teal-900/30', text: 'text-teal-400', label: 'Whale' },
  grassroots: { bg: 'bg-green-900/30', text: 'text-green-400', label: 'Grassroots' },
  artist_vs_artist: { bg: 'bg-amber-900/30', text: 'text-amber-400', label: 'Artist vs Artist' },
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-display text-amber-100">Rodeo Feed</h1>
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-700 hover:bg-amber-600 text-amber-50 text-sm font-semibold transition-colors shrink-0 shadow-sm"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-stone-700 bg-stone-900 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent"
        ...
        <select
          ...
          className="px-3 py-2.5 rounded-xl border border-stone-700 bg-stone-900 text-sm text-stone-300 focus:outline-none focus:ring-2 focus:ring-amber-500"
      ...
      {!isLoading && !error && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <Trophy className="w-12 h-12 text-stone-700 mb-3" aria-hidden="true" />
          <p className="text-stone-500 font-medium">No rodeos found</p>
          <p className="text-sm text-stone-600 mt-1">Check back later or adjust your filters</p>
...
function RodeoCard({ rodeo, onClick }: { rodeo: RodeoFeedItem; onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`w-full text-left bg-stone-900 rounded-2xl border shadow-sm p-5 transition-all hover:shadow-md group ${
        isLive ? 'border-amber-700 ring-1 ring-amber-500/20' : 'border-stone-700 hover:border-amber-800'
      }`}
    ...
    >
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        {isLive && countdown && (
          <span className="ml-auto flex items-center gap-1 text-xs font-semibold text-amber-400 bg-amber-950/20 px-2 py-0.5 rounded-full">
      ...
      {/* Title */}
      <h3 className="font-bold text-white text-lg leading-tight group-hover:text-amber-400 transition-colors mb-1">
        {rodeo.title}
      </h3>

      {/* Description */}
      {rodeo.description && (
        <p className="text-sm text-stone-500 line-clamp-2 mb-3">{rodeo.description}</p>
      )}

      {/* Matchup */}
      {entryNames.length >= 2 && (
        <div className="flex items-center gap-2 mb-4">
          <span className="text-sm font-semibold text-stone-100 truncate max-w-[40%]">
            {entryNames[0]}
          </span>
          <span className="flex-shrink-0 text-xs font-bold text-amber-400 bg-amber-950/20 px-2 py-0.5 rounded-full">
            VS
          </span>
          <span className="text-sm font-semibold text-stone-100 truncate max-w-[40%]">
            {entryNames[1]}
      ...
      {entryNames.length === 1 && (
        <div className="flex items-center gap-2 mb-4">
          <Users className="w-3.5 h-3.5 text-stone-600" aria-hidden="true" />
          <span className="text-sm text-stone-400">{entryNames[0]}</span>
          <span className="text-xs text-stone-600">— awaiting challenger</span>
        </div>
      )}
      ...
      <div className="flex items-center gap-4 pt-3 border-t border-stone-800">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs text-stone-600">pool</span>
        </div>
        ...
        {(rodeo.status === 'voting' || rodeo.status === 'closed' || rodeo.status === 'archived') && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-sm font-semibold text-stone-300">{totalVotes}</span>
            <span className="text-xs text-stone-600">votes</span>
          </div>
        )}

        {/* Arrow */}
        <ChevronRight className="w-4 h-4 text-stone-700 ml-auto group-hover:text-amber-400 transition-colors" aria-hidden="true" />
      </div>
    </button>
  )
}
